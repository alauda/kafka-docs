...
- **Intra-cluster/Inter-cluster Access**: Whether the application and Kafka server are within the same business cluster.
...
| Inter-cluster | Disabled                               | Disabled              | Not recommended                                 |
| Inter-cluster | Disabled                               | Enabled               | Testing scenario                               |
| Inter-cluster | Enabled SCRAM-SHA-512                  | Disabled              | Access through the external network, messages are transmitted without encryption                       |
| Inter-cluster | Enabled SCRAM-SHA-512 or TLS (recommended) | Enabled            | Access through the external network, messages are transmitted with encryption, and authentication is required for sending and receiving messages. (Higher security, performance may be affected) |
...
| Intra-cluster/Inter-cluster Connection | The application selects the required connection address based on its location. |
...
- Inter-cluster access, with authentication (SCRAM-SHA-512 protocol), non-encrypted transmission
- Inter-cluster access, with authentication (SCRAM-SHA-512 protocol), encrypted transmission
- Inter-cluster access, with authentication (TLS protocol), encrypted transmission
...
### 3. Inter-cluster Access - With Authentication (SCRAM-SHA-512 Protocol), Non-encrypted Transmission
...
1. You can copy the connection address for "Inter-cluster Access - Plain Transmission" under the 【Kafka Instance - Access Method】 tab and replace it in the actual code snippet.
...
### 4. Inter-cluster Access - With Authentication (SCRAM-SHA-512 Protocol), Encrypted Transmission
