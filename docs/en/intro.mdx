---
weight: 10
sourceSHA: a1e21bc307c8e21b4cefbae8a4b60252c3265215bf597065d14171c57cef8d92
---
...
- **ZooKeeper (zk)**: The traditional mode, relying on ZooKeeper to manage cluster metadata.

## What is Alauda Streaming Service for Kafka?

**Alauda Streaming Service for Kafka** is a deeply customized solution developed based on Kubernetes (K8s), providing convenience for the full lifecycle management of Kafka instances in Kubernetes clusters through a custom controller (Operator). It extends the Kubernetes API to provide users with a declarative configuration approach, automating the operation and maintenance of Kafka clusters, allowing users to deploy, manage, and maintain Kafka clusters easily in a cloud-native manner, thereby meeting the stringent demands for stability, scalability, and efficiency in production environments.

### Core Features

1. **zk Deployment Mode Support**

   | Mode        | Applicable Scenarios             | Core Advantages                                                          |
   | --------- | ----------------------------- | --------------------------------------------------------------------- |
   | ZooKeeper | Suitable for various sizes of production and non-production environments | ZooKeeper maintains metadata information for the Kafka cluster, including node status, topic partition allocation, etc., ensuring stable operation and coordination of the cluster. |
...
4. **Networking and Access Methods**
   - Supports various Kubernetes service types, such as NodePort and LoadBalancer, allowing users to expose Kafka services to external applications based on their network architecture and needs. Additionally, users can flexibly specify NodePort ports to meet specific network configuration requirements.
...
5. **Elastic Scaling**
   - Supports online horizontal scaling, dynamically adding or removing Kafka Broker nodes without service interruption to meet the changing business demands. For example, nodes can be added during peak business periods to enhance cluster processing capacity, while nodes can be reduced during lulls to save resources.
...
7. **Custom Scheduling Policies**
   - Supports Kubernetes' NodeSelector, Toleration, and Affinity scheduling policies, allowing users to flexibly control the scheduling logic of Kafka Pods based on node labels, characteristics, and business needs, thus deploying Kafka instances on the most suitable nodes to maximize cluster resources.
...
8. **Automated Operation and Maintenance Capabilities**
   ...
   - **Configuration Hot Update**: Users only need to modify the relevant ConfigMap configuration files, and the system will automatically inject the new configuration into the Kafka instance without needing to restart the Kafka service, achieving dynamic configuration updates and improving operation efficiency.

## Why Choose Alauda Streaming Service for Kafka
