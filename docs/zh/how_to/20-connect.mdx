...
3. TLS 协议的用户及密码证书
    ...
    * 生成用户证书：
        ```
        $ kubectl -n <namespace> get secret <Kafka 用户名称> -o jsonpath='{.data.user\.p12}' | base64 -d > user.p12
        ```

    * 生成用户证书的密码：
        ```
        $ kubectl -n <namespace> get secret <Kafka 用户名称> -o jsonpath='{.data.user\.password}' | base64 -d
        ```
